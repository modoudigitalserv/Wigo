...
import { TrendingUp, UserRound, Building2, Users, Calendar, Settings, ShieldCheck, ShieldAlert, CheckCircle2, Clock } from "lucide-react";
...
export default async function AdminCompaniesPage() {
  ...
  const navLinks = [
    ...
    { href: "/dashboard/companies", label: "Entreprises", icon: Building2, active: true },
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-black text-zinc-50 font-sans pt-16">
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold">Entreprises de Location</h1>
            <p className="text-zinc-500 mt-1">Gérez les loueurs inscrits sur la plateforme.</p>
          </div>
        </div>

        <div className="space-y-4">
          {allCompanies.map((company: any) => {
            const owner = company.profiles;
            const plan = company.subscription_plan || "basic";
            const planColors: Record<string, string> = {
              basic: "text-zinc-400 border-zinc-800 bg-zinc-900",
              pro: "text-blue-400 border-blue-500/20 bg-blue-600/10",
              premium: "text-purple-400 border-purple-500/20 bg-purple-600/10",
            };
            const planStyle = planColors[plan] || planColors.basic;

            return (
              <Card key={company.id} className="glass-card border-zinc-800/50 bg-zinc-950/80 hover:border-zinc-700 transition-colors">
                    ...
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div>
                        <h3 className="font-bold text-white flex items-center gap-2">
                          {company.name}
                          {company.is_verified && <ShieldCheck className="w-4 h-4 text-emerald-500" />}
                        </h3>
                        <div className="flex items-center gap-4 mt-1 text-sm text-zinc-400">
                          <span className="flex items-center gap-1">📍 {company.city || "Non spécifiée"}</span>
                          <span className="flex items-center gap-1">📞 {company.phone || "Non spécifié"}</span>
                        </div>
                        <p className="text-xs text-zinc-500 mt-1">Propriétaire: {owner?.full_name || owner?.email}</p>
                      </div>
                    </div>
          ...
          {allCompanies.length === 0 && (
             <div className="text-center py-20 bg-zinc-900/30 rounded-2xl border border-zinc-800/50">
               ...
               <h3 className="text-xl font-bold text-zinc-400 mb-2">Aucune entreprise</h3>
               <p className="text-zinc-600">Il n&apos;y a pas encore d&apos;entreprise inscrite.</p>
             </div>
          )}
        </div>
      </main>
